# Adapted from
# https://github.com/CSBDeep/CSBDeep/blob/master/csbdeep/scripts/care_predict.py
...
def parse_args():
    ...
    data.add_argument('--overview_plane',     metavar='', type=str2bool, required=False,  const=True, nargs='?', default=False,                                            help="input file contains an overview plane")
...
def main():
    ...
    if args.model_type is None or args.model_type == 'care':
        print('Model type = care')
        model = CARE(config=None, name=args.model_name, basedir=args.model_basedir)
    elif args.model_type == 'niklas_net_v2':
        import niklas_net_v2
        model =  niklas_net_v2.Niklas_Net(config=None, name=args.model_name, basedir=args.model_basedir)
