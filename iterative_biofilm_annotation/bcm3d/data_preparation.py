...
def bcm3d_wrapper(label_path: Path) -> None:
    ...
    output_dir_ext = label_path.parent.parent / 'target_bcm3d_1'
    output_dir_ext.mkdir(parents=True, exist_ok=True)
    
    output_dir_next = label_path.parent.parent / 'target_bcm3d_2'
...
def main() -> None:
    args = parse_args()
    label_paths = sorted(args.input_dir.glob('*.tif'))
